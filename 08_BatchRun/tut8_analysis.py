...
def setPlotFormat(numColors=8):
    ...
    colorlist = [colormap(1.0 * i / NUM_COLORS) for i in range(NUM_COLORS)]
...
def plot2DRate(
    ...
    for k in list(df.popRates[0].keys()):
        dfpop[k] = [r[k] for r in df.popRates]
    # return dfpop
...
# --------------------------------------------------------------------
def readPlot():
    dataFolder = "results/Tutorial08"
    batchLabel = "tauWeight"

    params, data = readBatchData(
        dataFolder, batchLabel, loadAll=False, saveAll=1, vars=None, maxCombs=None
    )
    print("Params:", params)
    print("Data:", data)
    plot2DRate(
