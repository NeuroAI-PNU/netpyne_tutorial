...
import SimpleNet as SN  # neural network designed through netpyne, to be optimized
...
def generate_netparams(random, args):
    ...
    initialParams = [
        random.uniform(minParamValues[i], maxParamValues[i]) for i in range(size)
    ]
...
def evaluate_netparams(candidates, args):
    ...
    for icand, cand in enumerate(candidates):
        ...
        SN.netParams.connParams["S->M"]["probability"] = cand[0]
        SN.netParams.connParams["S->M"]["weight"] = cand[1]
        SN.netParams.connParams["S->M"]["delay"] = cand[2]

        # create network
        sim.createSimulate(netParams=SN.netParams, simConfig=SN.simConfig)

        # calculate firing rate
        numSpikes = float(len(sim.simData["spkt"]))
        numCells = float(len(sim.net.cells))
        duration = SN.simConfig.duration / 1000.0
        netFiring = numSpikes / numCells / duration

        # calculate fitness for this candidate
        fitness = abs(
            targetFiring - netFiring
        )  # minimize absolute difference in firing rate
...
my_ec.selector = (
    ec.selectors.tournament_selection
)  # tournament sampling of individuals from population (<num_selected> individuals are chosen based on best fitness performance in tournament)

# toggle variators
my_ec.variator = [
    ec.variators.uniform_crossover,  # biased coin flip to determine whether 'mom' or 'dad' element is passed to offspring design
...
)  # existing generation is replaced by offspring, with elitism (<num_elites> existing individuals will survive if they have better fitness than offspring)
...
# toggle observers
my_ec.observer = [
    ec.observers.stats_observer,  # print evolutionary computation statistics
    ec.observers.plot_observer,  # plot output of the evolutionary computation as graph
    ec.observers.best_observer,
]  # print the best individual in the population to screen

# call evolution iterator
final_pop = my_ec.evolve(
    generator=generate_netparams,  # assign design parameter generator to iterator parameter generator
...
SN.simConfig.analysis["plotRaster"] = True  # plotting
SN.netParams.connParams["S->M"]["probability"] = bestCand[
    0
]  # set SN values to corresponding best candidate values
SN.netParams.connParams["S->M"]["weight"] = bestCand[1]  #
SN.netParams.connParams["S->M"]["delay"] = bestCand[2]  #
sim.createSimulateAnalyze(
    netParams=SN.netParams, simConfig=SN.simConfig
)  # run simulation of best candidate
